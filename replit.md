...
-   **Release gating:** Run `python -m release_preflight --approved-sha <full-lowercase-40-character-SHA>` before a human Publish. `RELEASE READY: YES` requires a clean workspace at that exact SHA, successful GitHub `Tests` and `Source integrity` check runs attached to that exact SHA, and all existing environment/database/schema safeguards. GitHub evidence is read-only and fails closed when absent, stale, unsuccessful, or unavailable. A pass authorizes only a manual Publish; it never deploys automatically.
-   **Post-Publish verification:** After a future manual Publish, rerun the same command with `--health-url https://<published-host>/health`. In addition to the pre-publish checks, it requires a healthy Production response with `release_identity_status=VERIFIED` and `release_sha` equal to the approved CI SHA. The deployment build continues to record its clean source `HEAD` in `build/release.sha`; no release metadata is fabricated by preflight.
-   **Migration target safety:** Migration tooling requires `BASELODGE_MIGRATION_MODE=1` and an explicit `BASELODGE_MIGRATION_TARGET` of `replit`, `supabase-development`, or `supabase-production`. Replit uses `DATABASE_URL`; Supabase Development uses `BASELODGE_DEVELOPMENT_DATABASE_URL`; Supabase Production is anchored to the verified live `SUPABASE_DATABASE_URL` Session Pooler and independently validates its historical endpoint identity plus protected project reference. A contradictory `BASELODGE_PRODUCTION_DATABASE_URL` fails closed. `BASELODGE_MIGRATION_DATABASE_URL` never authorizes migrations.
-   **Migration identities:** Replit and Supabase Development require their independent expected identity hashes: `BASELODGE_MIGRATION_REPLIT_IDENTITY_HASH` or `BASELODGE_MIGRATION_SUPABASE_DEVELOPMENT_IDENTITY_HASH`. Supabase Production does not accept a self-consistent URL/companion-hash pair as ownership proof; it requires `BASELODGE_PRODUCTION_DATABASE_IDENTITY_HASH`, `BASELODGE_PRODUCTION_SUPABASE_PROJECT_REF_HASH`, and `BASELODGE_CONFIRM_PRODUCTION_MIGRATION=1`. Do not place database URLs, credentials, or raw Supabase project references in source or command history.
-   **DIAGNOSTIC ONLY — Replit:** `BASELODGE_RUNTIME_ENV=development BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=replit python scripts/database_target.py diagnose`
-   **DIAGNOSTIC ONLY — Supabase Development:** `BASELODGE_RUNTIME_ENV=development BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=supabase-development python scripts/database_target.py diagnose`
-   **DIAGNOSTIC ONLY — Supabase Production:** `BASELODGE_RUNTIME_ENV=production BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=supabase-production BASELODGE_CONFIRM_PRODUCTION_MIGRATION=1 python scripts/database_target.py diagnose`
-   **Optional read-only revision check:** Append `--check-revision` to a diagnostic command only when a live connection is intentionally authorized. The command starts a read-only transaction, verifies read-only mode, and reads only `alembic_version`; it never runs migration operations.
-   **MIGRATION EXECUTION — Replit:** After a passing diagnostic and separate operator approval: `BASELODGE_RUNTIME_ENV=development BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=replit alembic -c migrations/alembic.ini upgrade head`
-   **MIGRATION EXECUTION — Supabase Development:** After BL-135 backup/dry-run/rollback validation and separate operator approval: `BASELODGE_RUNTIME_ENV=development BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=supabase-development alembic -c migrations/alembic.ini upgrade head`
-   **MIGRATION EXECUTION — Supabase Production:** Requires separate production authorization and all three gates: `BASELODGE_RUNTIME_ENV=production BASELODGE_MIGRATION_MODE=1 BASELODGE_MIGRATION_TARGET=supabase-production BASELODGE_CONFIRM_PRODUCTION_MIGRATION=1 alembic -c migrations/alembic.ini upgrade head`
-   **Alembic enforcement:** These documented commands are operator guidance, not the security boundary. `migrations/env.py` invokes the centralized resolver before importing models, configuring offline Alembic, creating an engine, or opening a connection, so raw Alembic and Flask-Migrate commands cannot bypass target verification.

...
-   **Migration target safety:** Migration tooling requires `BASELODGE_MIGRATION_MODE=1` and an explicit `BASELODGE_MIGRATION_TARGET` of `replit`, `supabase-development`, or `supabase-production`. The target selects exactly one URL source: Replit uses `DATABASE_URL`, Supabase Development uses `BASELODGE_DEVELOPMENT_DATABASE_URL`, and Supabase Production uses `BASELODGE_PRODUCTION_DATABASE_URL`. `BASELODGE_MIGRATION_DATABASE_URL` and `SUPABASE_DATABASE_URL` never authorize migrations.
-   **Migration identities:** Configure the selected target's independent expected identity hash: `BASELODGE_MIGRATION_REPLIT_IDENTITY_HASH`, `BASELODGE_MIGRATION_SUPABASE_DEVELOPMENT_IDENTITY_HASH`, or `BASELODGE_MIGRATION_SUPABASE_PRODUCTION_IDENTITY_HASH`. Production additionally requires `BASELODGE_CONFIRM_PRODUCTION_MIGRATION=1`. Do not place database URLs, credentials, or raw Supabase project references in source or command history.

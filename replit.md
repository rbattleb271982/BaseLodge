...
- **Authentication & Onboarding:** Two-step flow: (1) Minimal signup collects only first name, last name, email, password. (2) Full-screen Identity Setup immediately after signup collects rider_types (multi-select), skill_level (radio), and pass_type (multi-select chips). Progressive modals on Home (1st-2nd logins only) collect equipment (status + optional gear) and riding style. Welcome screen with 3 CTAs (Add Trip, Invite Friends, Explore) appears once after follow-ups complete. Stop conditions: `welcome_next_steps_shown_at` set OR `login_count > 2` = no more modals. User model computed properties: `is_core_profile_complete`, `is_equipment_complete`, `is_profile_complete`, `should_show_progressive_modal`. Renter-friendly flow with optional brand/model fields.

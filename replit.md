...
- **Authentication & Onboarding:** Two-step flow: (1) Minimal signup collects only first name, last name, email, password. (2) Full-screen Identity Setup immediately after signup collects rider_types (multi-select), skill_level (radio), and pass_type (multi-select chips). Simplified Welcome modal (single "Done" CTA) appears once after signup. Stop condition: `welcome_next_steps_shown_at` set. User model computed properties: `is_core_profile_complete`, `is_equipment_complete`, `is_profile_complete`. Equipment is optional and managed in Settings > Equipment.
...
- **Group Coordination Signals:** SkiTripParticipant includes transportation_status and equipment_status for per-participant trip coordination. Trip Detail page shows aggregated Group Signals summary card.

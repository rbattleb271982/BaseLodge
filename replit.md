...
- **Authentication & Onboarding:** Low-friction signup collects rider_types (multi-select), skill_level, and pass_type upfront. Post-login progressive modals collect equipment (Step 1) and riding style (Step 2) on 1st-2nd logins only. Welcome screen with CTAs appears after profile completion. Stop conditions: profile complete OR 2nd login reached = no more modals. User model has `is_equipment_complete`, `is_profile_complete`, `should_show_progressive_modal` computed properties. Renter-friendly flow with optional brand/model fields.

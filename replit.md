...
- **Equipment Management:** Users add/edit Primary and Secondary equipment setups (Discipline, Brand, Model, Length, Width, Binding Type, Boot Brand, Boot Model, Boot Flex) via settings. Model fields are optional and displayed in "Brand Model" format throughout the app. Users can set a global `equipment_status` (`have_own_equipment`, `needs_rentals`) with per-trip overrides.

...
def handle_exception(e):
    ...
    import sys
    print("=" * 70, flush=True)
    print(f"🚨 UNHANDLED EXCEPTION: {type(e).__name__}", flush=True)
    print("=" * 70, flush=True)
    ...
    try:
        print(f"Request Host: {request.host}", flush=True)
    except Exception as host_err:
        print(f"Request Host: ERROR GETTING HOST - {host_err}", flush=True)
    print(f"Host Header: {request.headers.get('Host', 'NOT SET')}", flush=True)
    print(f"X-Forwarded-Host: {request.headers.get('X-Forwarded-Host', 'NOT SET')}", flush=True)
    print(f"User Agent: {request.headers.get('User-Agent', 'NOT SET')}", flush=True)
    print(f"Endpoint: {request.endpoint}", flush=True)
    print(f"Is Authenticated: {current_user.is_authenticated if hasattr(current_user, 'is_authenticated') else 'N/A'}", flush=True)

...
**How to apply:**
For bounded remember age, use a versioned timed envelope and reject untimed legacy remember cookies. If the boundary is exact, protect a high-precision issuance instant inside the envelope because itsdangerous timestamps are whole seconds; derive browser expiry from that same instant. Do not reuse `password_changed_at` as a general revocation marker: current identity versioning does not include it, and it also carries password-history/reset-token semantics.